...
public partial class ReservaPage : ContentPage
{
    ...
    // NUEVO: Refresca las horas disponibles cada vez que el usuario vuelve a esta pantalla
    protected override async void OnAppearing()
    {
        base.OnAppearing();

        if (diaSeleccionado != null && barberoSeleccionado != null)
        {
    ...
    private async Task ActualizarHorasDisponibles()
    {
        ...
        try
        {
            ...
            // MEJORA: Busca las reservas del barbero seleccionado Y también las que están como "Cualquiera"
            var filtro = Builders<ReservaModel>.Filter.And(
                Builders<ReservaModel>.Filter.In(r => r.Barbero, new[] { barberoSeleccionado.Nombre, "Cualquiera" }),
        ...
        {
            // SOLUCIÓN AL CRASHEO Y A LAS RESERVAS FANTASMA: Si falla la conexión, vaciamos la lista y avisamos.
            todasHoras = new List<HoraReserva>();
            BindableLayout.SetItemsSource(ListaHoras, todasHoras);
            await DisplayAlert("Error Real", ex.ToString(), "OK");
        }
    }
    ...
    {
        // PROTECCIÓN EXTRA: Si no hay horas cargadas, no hacemos nada.
        if (todasHoras == null) return;
